...
if [ $? -eq 0 ]; then
	...
	printf [account]"\n"$newaccesskey"\n"$newsecretkey"\n""\n" > /tmp/credentials
		if [ $? -eq 0 ]; then
			...
			tput bold
			echo "Failed to create new key in "$account" too many keys, or must be logged in using MFA"
			tput sgr0
		fi;
	...
	AWS_ACCES_KEY_ID=$newaccesskey AWS_SECRET_ACCESS_KEY=$newsecretkey aws sts get-caller-identity;
		if [ $? -eq 0 ]; then
			echo "Successful"
		else 
			tput bold
